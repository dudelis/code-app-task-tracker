...
def get_credential():
    ...
    if client_id and client_secret:
        ...
        # Path 2: Reuse an existing interactive login (Azure CLI / PAC CLI) and fall
        # back to device code flow only if neither is available. AzureCliCredential
        # silently reuses the `az login` session, avoiding the device code prompt.
        ...
        try:
            ...
            chain.append(AzureCliCredential(tenant_id=tenant_id))
        except Exception:
            pass  # az CLI not available — rely on device code flow
        chain.append(device_code_credential)

        _credential = ChainedTokenCredential(*chain)
